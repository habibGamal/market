...
class PendingOrderSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $customers->each(function (Customer $customer) use ($products, $orderServices, $stockServices) {
            // Create a pending order
            $pendingOrder = Order::factory()
                ->create(['customer_id' => $customer->id, 'created_at' => now()->subDay(), 'status' => 'pending',]);
            ...
            for ($i = 0; $i < fake()->numberBetween(5, 8); $i++) {
                ...
                $stockServices->reserve($product, $pendingOrderItems[$i]['packets_quantity'] * $product->packet_to_piece + $pendingOrderItems[$i]['piece_quantity']);
            }
